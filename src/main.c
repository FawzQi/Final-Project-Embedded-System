...
void button_task(void* pv) {
    while (1) {
        ...
        if (millis() - last_polling_time > 175) {
            if (x_val < 1300) {
                aim_pos[1] -= 1;
...
void app_main(void) {
    ...
    // Buat task
    xTaskCreatePinnedToCore(button_task, "button_task", 2048, NULL, 0, NULL, 0);
    xTaskCreatePinnedToCore(bird_task, "bird_task", 2048, NULL, 0, NULL, 1);
    xTaskCreatePinnedToCore(led_task, "led_task", 2048, NULL, 5, NULL, 1);
    xTaskCreatePinnedToCore(buzzer_task, "buzzer_task", 2048, NULL, 5, NULL, 0);
    i2c_lcd1602_clear(lcd_info);
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(10));
        if (isHit) {
            ...
            i2c_lcd1602_move_cursor(lcd_info, 0, 0);
            char line1[17];
            sprintf(line1, "Score:%d Lives:%d ", score, nyawa);
            i2c_lcd1602_write_string(lcd_info, line1);
            i2c_lcd1602_move_cursor(lcd_info, 0, 1);
            char line2[17];
            sprintf(line2, "High:%d buzz: %d", highscore, buzzer_state);
